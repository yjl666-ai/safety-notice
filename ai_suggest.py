...
def preprocess_image(image_path: str, max_size: int = 1024) -> str:
    """预处理图片:缩放到 max_size 内,压缩后返回 base64 字符串。

    手机原图通常 4000×3000,base64 编码后 5-15MB,不处理直接传 API 慢且费钱。
    缩放到 1024px(Qwen-VL 推荐尺寸)可减少 90%+ 体积,不影响识别精度。
    """
    if not HAS_PIL:
        # 无 PIL 时回退到原始 base64(不推荐)
...
def photos_to_hazards(photo_paths: list, api_key: str, base_url: str, model: str,
                     max_workers: int = 5) -> list:
    """N 张图 → N 条隐患描述(并发调 Qwen,跳过失败照片,保留位置信息)。

    变更(v0.4.1):
    - 并发调用 ThreadPoolExecutor,10 张图从 50s → ~5s
    - 失败照片直接跳过,不再传 fallback 文本给 DeepSeek
    - 保留 Qwen 的"位置"信息,格式: "隐患描述（位置）"
    """
    hazards = []
    failed = []
    ...
    def _process_one(path: str):
        """单张图处理(在线程池中执行)。"""
        data = call_qwen_vl(path, api_key, base_url, model)
        desc = (data.get("隐患描述") or "").strip()
        location = (data.get("位置") or "").strip()
        return desc, location

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_path = {executor.submit(_process_one, p): p for p in photo_paths}
        for future in as_completed(future_to_path):
            p = future_to_path[future]
            try:
                desc, location = future.result()
                if desc:
                    text = f"{desc}（{location}）" if location else desc
                    hazards.append(text)
                    sys.stderr.write(f"✓ [{len(hazards)}/{len(photo_paths)}] {Path(p).name}: {desc[:30]}\n")
                else:
                    failed.append(p)
                    sys.stderr.write(f"⚠ {Path(p).name}: 识别为空，已跳过\n")
                ...
                try:
                    ...
                    pass
                failed.append(p)
                sys.stderr.write(f"✗ {Path(p).name}: HTTP {e.code} {err_body}\n")
            except Exception as e:
                failed.append(p)
                sys.stderr.write(f"✗ {Path(p).name}: {e}\n")

    if failed:
        sys.stderr.write(
            f"\n⚠ {len(failed)}/{len(photo_paths)} 张照片识别失败，已跳过。"
            f"请人工补充这 {len(failed)} 条隐患。\n"
        )
    return hazards
...
def main():
    parser = argparse.ArgumentParser(
        description="智安通 v0.4.1 — 隐患 → 整改通知。文本输入或 Qwen-VL-Max 看图识别。",
...
""",
    ...
    parser.add_argument("--output", "-o", default=None, metavar="PREFIX",
                        help="输出文件前缀(默认使用时间戳,如 notice_20260720_143052)")
    parser.add_argument("--project", default=None, metavar="NAME",
                        help="项目名称,用于输出文件前缀(如 '上海视源')")
    parser.add_argument("--title", default=None, metavar="TITLE",
                        help="通知标题(默认 '安全隐患整改通知书')")
    ...
    if args.output:
        ...
        prefix = f"{args.project}_notice_{timestamp}"
    else:
        prefix = f"notice_{timestamp}"
    ...
    docx_path = Path(f"{prefix}.docx")
    sys.stderr.write(f"→ 出 docx: python generate_notice.py {yaml_path} {docx_path}\\n")
